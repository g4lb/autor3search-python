...
class Runner:
    ...
    def compile_gate(self, paths: Sequence[str]) -> Result:
        ...
        # The extra `[^/]` matters when a caller passes "." (this module's own
        # test does): compileall then reports files as "./bad.py", and a bare
        # `(^|/)\.` matches that leading "./" itself, excluding every file in
        # the tree instead of just dotfiles and dotdirs.
        return self.python_run("-m", "compileall", "-q", "-x", r"(^|/)\.[^/]", *paths)
    ...
    def pytest_gate(self) -> Result:
        ...
        return self.python_run(
            "-m", "pytest", "-q", "-p", "no:cacheprovider", "--benchmark-disable"
        )
...
def importable_modules(root: str | Path, matcher: Matcher) -> list[str]:
    ...
    benchmarks will.
    """
    from autor3search_python.discover import SKIP_DIRS, is_test_file
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        dirnames[:] = sorted(
            d
            for d in dirnames
            if d not in SKIP_DIRS and not d.endswith(".egg-info") and not d.startswith((".", "_"))
        )
        here = Path(dirpath)
        for name in filenames:
            ...
            if is_test_file(rel) or not matcher.match(rel):
                continue
            path = here / name
